"""Health check endpoint for the balance service."""
...
class HealthView(APIView):
    """GET /health — liveness + readiness probe for balance_service.
    ...
    def get(self, request: Request) -> Response:
        ...
        payload = {
            ...
            "message": (
                "Balance service is in the green — wallets flush, "
                "credits flowing, not a single negative balance in sight! 💰"
                if healthy
                else "Balance service is broke — database wallet is empty (unreachable). 💸"
            ),
