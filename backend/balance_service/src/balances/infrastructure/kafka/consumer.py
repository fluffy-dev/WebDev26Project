...
class BalanceEventConsumer:
    ...
    Routes messages to the appropriate use case based on the `event` field.
    Operates with enable.auto.commit=false; offsets are committed only after
    successful processing or an idempotent duplicate detection.
    ...
    def _handle_user_registered(self, payload: dict) -> None:
        ...
        Args:
            payload: Deserialised Kafka message payload.
        """
...
def build_consumer() -> BalanceEventConsumer:
    ...
    Returns:
        A fully configured BalanceEventConsumer.
    """
