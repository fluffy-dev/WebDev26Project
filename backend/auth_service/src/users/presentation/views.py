...
from users.infrastructure.models import ProfileImage
...
class AvatarListView(APIView):
    """GET /avatars — retrieves predefined avatar catalogue."""

    permission_classes = [AllowAny]

    def get(self, request: Request) -> Response:
        """Returns all configured profile images."""
        from users.presentation.serializers import AvatarSerializer
        
        avatars = ProfileImage.objects.all()
        data = [{"id": img.id, "image_url": img.image_url} for img in avatars]
        serializer = AvatarSerializer(data, many=True)
