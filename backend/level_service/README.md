...
LevelEntity (frozen dataclass)
  ...
  level_type   : str       — default or cat_running
  created_at   : datetime
  updated_at   : datetime
...
CREATE TABLE levels (
    ...
    level_type  TEXT NOT NULL DEFAULT 'default',
...
- `0002_seed_levels.py` seeds the 20 common/default typing levels.
- `0004_seed_cat_running_levels.py` seeds 10 cat-running levels after `level_type` exists.
- Existing databases will keep their common levels and gain the cat-running catalog on the next migration run.
